...
description: Anthropic 모범 사례에 따라 Gemini CLI 스킬을 만들고 관리합니다. 새 스킬 생성, skill-rules.json 수정, 트리거 패턴 이해, 훅 작업, 스킬 활성화 디버깅, 점진적 공개(progressive disclosure) 구현 시 사용하세요. 스킬 구조, YAML 프론트매터, 트리거 유형(키워드, 의도 패턴, 파일 경로, 콘텐츠 패턴), enforcement 레벨(block/suggest/warn), 훅 메커니즘(UserPromptSubmit/PreToolUse), 세션 추적, 500줄 규칙을 다룹니다.
---
...
자동 활성화 시스템이 있는 Gemini CLI에서 스킬을 만들고 관리하기 위한 포괄 가이드입니다. 500줄 규칙과 점진적 공개(progressive disclosure) 패턴을 포함한 Anthropic 공식 모범 사례를 따릅니다.
...
- **파일**: `.gemini/hooks/skill-activation-prompt.ts`
...
- **파일**: `.gemini/hooks/error-handling-reminder.ts`
...
**위치**: `.gemini/skills/skill-rules.json`
...
**위치:** `.gemini/skills/{skill-name}/SKILL.md`
...
echo '{"session_id":"test","prompt":"your test prompt"}' | \
  npx tsx .gemini/hooks/skill-activation-prompt.ts
```

**PreToolUse 테스트:**
```bash
cat <<'EOF' | npx tsx .gemini/hooks/skill-verification-guard.ts
...
**상태 파일:** `.gemini/hooks/state/skills-used-{session_id}.json`
...
- [ ] `.gemini/skills/{name}/SKILL.md`에 스킬 파일 생성
...
1. 프론트매터를 포함해 `.gemini/skills/{name}/SKILL.md` 생성
2. `.gemini/skills/skill-rules.json`에 엔트리 추가
3. `npx tsx` 명령으로 테스트
...
```bash
# UserPromptSubmit
echo '{"prompt":"test"}' | npx tsx .gemini/hooks/skill-activation-prompt.ts
...
**설정(Configuration):**
- `.gemini/skills/skill-rules.json` - 마스터 설정
- `.gemini/hooks/state/` - 세션 추적
- `.gemini/settings.json` - 훅 등록

**훅(Hooks):**
- `.gemini/hooks/skill-activation-prompt.ts` - UserPromptSubmit
- `.gemini/hooks/error-handling-reminder.ts` - Stop 이벤트(부드러운 리마인더)

**모든 스킬(All Skills):**
- `.gemini/skills/*/SKILL.md` - 스킬 콘텐츠 파일
