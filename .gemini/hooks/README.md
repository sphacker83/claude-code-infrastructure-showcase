...
스킬 자동 활성화, 파일 추적, 검증을 가능하게 하는 Gemini CLI 훅입니다.
...
```bash
# 두 파일 모두 복사
cp skill-activation-prompt.sh your-project/.gemini/hooks/
cp skill-activation-prompt.ts your-project/.gemini/hooks/

# 실행 가능하도록 설정
chmod +x your-project/.gemini/hooks/skill-activation-prompt.sh

# 의존성 설치
cd your-project/.gemini/hooks
npm install
```
...
```bash
# 파일 복사
cp post-tool-use-tracker.sh your-project/.gemini/hooks/

# 실행 가능하도록 설정
chmod +x your-project/.gemini/hooks/post-tool-use-tracker.sh
```
...
1. 먼저 **[GEMINI.md](../../GEMINI.md)**를 읽으세요
2. **항상 두 개의 필수 훅부터 시작**하세요
3. **Stop 훅 추가 전에는 반드시 확인**하세요 - 잘못 구성되면 작업을 막을 수 있습니다
4. **설정 후 검증:**
   ```bash
   ls -la .gemini/hooks/*.sh | grep rwx
   ```

**질문이 있나요?** [GEMINI.md](../../GEMINI.md)를 참고하세요.
