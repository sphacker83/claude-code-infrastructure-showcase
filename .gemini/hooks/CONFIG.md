...
### 1. .gemini/settings.json에 훅 등록

프로젝트 루트에 있는 `.gemini/settings.json`을 생성하거나 업데이트하세요:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.gemini/hooks/stop-prettier-formatter.sh"
          },
          {
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.gemini/hooks/stop-build-check-enhanced.sh"
          },
          {
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.gemini/hooks/error-handling-reminder.sh"
          }
        ]
      }
    ]
...
`.gemini/hooks/post-tool-use-tracker.sh`의 `detect_repo()` 함수를 수정하세요:
...
`.gemini/hooks/post-tool-use-tracker.sh`의 `get_build_command()` 함수를 수정하세요:
...
`.gemini/hooks/post-tool-use-tracker.sh`의 `get_tsc_command()` 함수를 수정하세요:
...
`.gemini/hooks/stop-prettier-formatter.sh`의 `get_prettier_config()` 함수를 수정하세요:
...
`.gemini/hooks/error-handling-reminder.ts`에서 파일 카테고리 감지를 구성하세요:
...
`.gemini/hooks/stop-build-check-enhanced.sh`를 수정하세요:
...
```
$GEMINI_PROJECT_DIR/.gemini/tsc-cache/[session_id]/
```
...
```bash
# 모든 캐시 데이터 제거
rm -rf $GEMINI_PROJECT_DIR/.gemini/tsc-cache/*

# 특정 세션 제거
rm -rf $GEMINI_PROJECT_DIR/.gemini/tsc-cache/[session-id]
```
...
1. **등록 확인:** `.gemini/settings.json`에 훅이 있는지 확인
2. **권한 확인:** `chmod +x .gemini/hooks/*.sh` 실행
3. **경로 확인:** `$GEMINI_PROJECT_DIR`가 올바르게 설정되어 있는지 확인
4. **TypeScript 확인:** `cd .gemini/hooks && npx tsc`를 실행해 에러가 있는지 확인
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$GEMINI_PROJECT_DIR/.gemini/hooks/my-custom-bash-guard.sh"
          }
        ]
      }
    ]
