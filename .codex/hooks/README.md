...
스킬 자동 활성화, 파일 추적, 검증을 가능하게 하는 Roo Codex 훅입니다.

---

## 훅이란?

훅은 Codex의 워크플로우에서 특정 시점에 실행되는 스크립트입니다:
- **UserPromptSubmit**: 사용자가 프롬프트를 제출할 때
- **PreToolUse**: 도구가 실행되기 전에
- **PostToolUse**: 도구 실행이 완료된 후
- **Stop**: 사용자가 중지를 요청할 때

**핵심 인사이트:** 훅은 프롬프트를 수정하고, 동작을 차단하고, 상태를 추적할 수 있어 Codex만으로는 할 수 없는 기능을 가능하게 합니다.
...
```bash
# 두 파일 모두 복사
cp skill-activation-prompt.sh your-project/.codex/hooks/
cp skill-activation-prompt.ts your-project/.codex/hooks/

# 실행 가능하도록 설정
chmod +x your-project/.codex/hooks/skill-activation-prompt.sh

# 의존성 설치
cd your-project/.codex/hooks
npm install
```
...
**필수인 이유:** Codex가 코드베이스에서 어떤 부분이 활성화되어 있는지 이해하는 데 도움이 됩니다.

**통합:**
```bash
# 파일 복사
cp post-tool-use-tracker.sh your-project/.codex/hooks/

# 실행 가능하도록 설정
chmod +x your-project/.codex/hooks/post-tool-use-tracker.sh
```
