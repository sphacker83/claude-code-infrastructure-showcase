...
function findProjectDir(input: HookInput): string {
    ...
    for (const candidate of candidates) {
        ...
        const rulesPath = join(normalized, '.gemini', 'skills', 'skill-rules.json');
...
function getRecentEdit(projectDir: string, sessionId?: string): RecentEdit | null {
    ...
    const logPath = join(projectDir, '.gemini', 'tsc-cache', sessionId, 'edited-files.log');
...
async function main() {
    try {
        ...
        const rulesPath = join(projectDir, '.gemini', 'skills', 'skill-rules.json');
        if (!existsSync(rulesPath)) {
            process.exit(0);
            return;
        }
