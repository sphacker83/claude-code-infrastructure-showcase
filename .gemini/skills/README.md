...
컨텍스트에 따라 자동 활성화되는, 프로덕션에서 검증된 Gemini CLI용 스킬 모음입니다.
...
전체 안내는 [GEMINI.md](../../GEMINI.md)를 참고하세요.

### 수동 통합

**1단계: 스킬 디렉터리 복사**
```bash
cp -r codex-code-infrastructure-showcase/.gemini/skills/backend-dev-guidelines \\
...
없다면 생성하세요:
```bash
cp codex-code-infrastructure-showcase/.gemini/skills/skill-rules.json \\
   your-project/.gemini/skills/
```
...
**확인:**
1. 스킬 디렉터리가 `.gemini/skills/`에 있나요?
...
# 훅 수동 실행 테스트
./.gemini/hooks/skill-activation-prompt.sh
```
...
**질문이 있나요?** 더 자세한 통합 가이드는 [GEMINI.md](../../GEMINI.md)를 참고하세요.
