...
echo "CODEX_PROJECT_DIR: $CODEX_PROJECT_DIR" >> /tmp/claude-hook-debug.log
...
for service in "${services_dirs[@]}"; do
    service_path="$CODEX_PROJECT_DIR/$service"
...
if [[ ${#services_with_changes[@]} -gt 0 ]]; then
    ...
    # Use the correct Codex CLI syntax - try different options
...
EOF
        ...
        echo "Codex CLI not found in PATH" >> /tmp/claude-hook-debug.log
    fi
    
    echo "Codex command completed with exit code: $?" >> /tmp/claude-hook-debug.log
else
