...
**CODEX CLI용:** 사용자가 이 쇼케이스 저장소의 구성요소를 자신의 프로젝트에 통합해 달라고 요청하면, 아래 절차를 따르세요.

---

## 개요

이 저장소는 Codex CLI 인프라 구성요소를 모아둔 **레퍼런스 라이브러리**입니다. 사용자는 특정 조각을 프로젝트에 적용하려고 합니다. 당신의 역할은 다음과 같습니다.
...
```bash
cp -r /path/to/showcase/.codex/skills/[skill-name] \
      $CODEX_PROJECT_DIR/.codex/skills/
```
...
**파일 존재 여부 확인:**
```bash
ls $CODEX_PROJECT_DIR/.codex/skills/skill-rules.json
```
...
```bash
# 스킬 복사 확인
ls -la $CODEX_PROJECT_DIR/.codex/skills/[skill-name]

# skill-rules.json 문법 확인
cat $CODEX_PROJECT_DIR/.codex/skills/skill-rules.json | jq .
```
...
1. 시작점으로 스킬 복사
```bash
cp -r showcase/.codex/skills/frontend-dev-guidelines \
      $CODEX_PROJECT_DIR/.codex/skills/vue-dev-guidelines
```
...
```bash
# 두 파일 복사
cp showcase/.codex/hooks/skill-activation-prompt.sh \
   $CODEX_PROJECT_DIR/.codex/hooks/
cp showcase/.codex/hooks/skill-activation-prompt.ts \
   $CODEX_PROJECT_DIR/.codex/hooks/

# 실행 권한 부여
chmod +x $CODEX_PROJECT_DIR/.codex/hooks/skill-activation-prompt.sh

# 필요 시 의존성 설치
if [ -f "showcase/.codex/hooks/package.json" ]; then
  cp showcase/.codex/hooks/package.json \
     $CODEX_PROJECT_DIR/.codex/hooks/
  cd $CODEX_PROJECT_DIR/.codex/hooks && npm install
fi
```
...
```bash
# 파일 복사
cp showcase/.codex/hooks/post-tool-use-tracker.sh \
   $CODEX_PROJECT_DIR/.codex/hooks/

# 실행 권한 부여
chmod +x $CODEX_PROJECT_DIR/.codex/hooks/post-tool-use-tracker.sh
```
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/post-tool-use-tracker.sh"
          }
        ]
      }
    ]
...
에이전트 파일은 독립형이지만, **Codex에서는 자동 주입되지 않습니다.**
즉, `.md`를 복사한 뒤 호출 시 프롬프트에 명시적으로 주입해야 합니다.

### 기본 통합

```bash
cp showcase/.codex/agents/[agent-name].md \
   $CODEX_PROJECT_DIR/.codex/agents/

# Codex용 호출 보조 파일도 함께 복사
cp showcase/.codex/agents/manifest.json \
   $CODEX_PROJECT_DIR/.codex/agents/
cp showcase/.codex/agents/compile-agent-prompt.mjs \
   $CODEX_PROJECT_DIR/.codex/agents/
```
...
```bash
node $CODEX_PROJECT_DIR/.codex/agents/compile-agent-prompt.mjs \
  --agent [agent-name] \
  --task "[task]" \
  --ownership "[scope]"
```

출력된 `AGENT_SPEC_BEGIN ... AGENT_SPEC_END` 블록을 `Task/spawn_agent`의 `message`에 그대로 포함해 호출합니다.

### 누락 방지(권장)

`PreToolUse` 훅에 `agent-instruction-guard.sh`를 추가해 spec 미주입 호출을 차단합니다.

### 하드코딩 경로 점검

일부 에이전트는 경로를 직접 적어둘 수 있습니다. 복사 전 다음 패턴을 확인하세요.

- `~/git/old-project/` → `$CODEX_PROJECT_DIR` 또는 `.`로 변경
- `/root/git/project/` → `$CODEX_PROJECT_DIR` 또는 `.`로 변경
- 스크린샷 저장 경로 하드코딩 → 사용자에게 원하는 저장 위치 확인

필요 시 치환 예시:
```bash
sed -i 's|~/git/old-project/|.|g' $CODEX_PROJECT_DIR/.codex/agents/[agent].md
sed -i 's|/root/git/.*PROJECT.*DIR|$CODEX_PROJECT_DIR|g' \
    $CODEX_PROJECT_DIR/.codex/agents/[agent].md
```
...
```bash
cp showcase/.codex/commands/[command].md \
   $CODEX_PROJECT_DIR/.codex/commands/
```
...
```bash
# 1. 훅 실행 권한
ls -la $CODEX_PROJECT_DIR/.codex/hooks/*.sh
# 기대값: -rwxr-xr-x
...
ls $CODEX_PROJECT_DIR/.codex/hooks/node_modules/
# package.json이 있다면 패키지 목록이 보여야 함

# 4. settings.json 유효성
cat $CODEX_PROJECT_DIR/.codex/settings.json | jq .
# 오류 없이 파싱되어야 함
```
...
현재 .codex/settings.json이 이미 있나요, 아니면 새로 만들까요?
```
...
Codex에서는 에이전트 md가 자동 주입되지 않으므로,
manifest/compile 스크립트와 함께 복사하고, compile 출력을 Task/spawn_agent에 주입하도록 설정하겠습니다.
```
...
사용 방법:
1) compile-agent-prompt 출력물을 생성
2) 출력된 AGENT_SPEC 블록을 Task/spawn_agent message에 주입
...
| **모든 에이전트** | 중간 | ⚠️ 호출 주입 필요 | "어떤 에이전트를 어떤 범위로 실행?" |
