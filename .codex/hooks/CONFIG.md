...
### 1. .codex/settings.json에 훅 등록

프로젝트 루트에 있는 `.codex/settings.json`을 생성하거나 업데이트하세요:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/stop-prettier-formatter.sh"
          },
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/stop-build-check-enhanced.sh"
          },
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/error-handling-reminder.sh"
          }
        ]
      }
    ]
...
`.codex/hooks/post-tool-use-tracker.sh`의 `detect_repo()` 함수를 수정하세요:
...
`.codex/hooks/post-tool-use-tracker.sh`의 `get_build_command()` 함수를 수정하세요:
...
`.codex/hooks/post-tool-use-tracker.sh`의 `get_tsc_command()` 함수를 수정하세요:
...
`.codex/hooks/stop-prettier-formatter.sh`의 `get_prettier_config()` 함수를 수정하세요:
...
`.codex/hooks/error-handling-reminder.ts`에서 파일 카테고리 감지를 구성하세요:
...
`.codex/hooks/stop-build-check-enhanced.sh`를 수정하세요:
...
export CODEX_PROJECT_DIR=/path/to/your/project
```

### 세션별 환경 변수

Roo Codex를 시작하기 전에 설정하세요:
...
```
$CODEX_PROJECT_DIR/.codex/tsc-cache/[session_id]/
```
...
```bash
# 모든 캐시 데이터 제거
rm -rf $CODEX_PROJECT_DIR/.codex/tsc-cache/*

# 특정 세션 제거
rm -rf $CODEX_PROJECT_DIR/.codex/tsc-cache/[session-id]
```
...
1. **등록 확인:** `.codex/settings.json`에 훅이 있는지 확인
2. **권한 확인:** `chmod +x .codex/hooks/*.sh` 실행
3. **경로 확인:** `$CODEX_PROJECT_DIR`가 올바르게 설정되어 있는지 확인
4. **TypeScript 확인:** `cd .codex/hooks && npx tsc`를 실행해 에러가 있는지 확인
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CODEX_PROJECT_DIR/.codex/hooks/my-custom-bash-guard.sh"
          }
        ]
      }
    ]
