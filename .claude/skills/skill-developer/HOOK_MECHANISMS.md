...
```
Claude가 Edit/Write 툴을 호출
    ↓
.claude/settings.json이 훅을 등록(matcher: Edit|Write)
    ↓
...
그 외:
  종료 코드 0으로 종료(ALLOW)
    ↓
차단(BLOCK)된 경우:
  stderr → Claude가 메시지를 봄
  Edit/Write 툴이 실행되지 않음
  Claude가 스킬을 사용한 뒤 재시도해야 함
허용(ALLOW)된 경우:
  툴이 정상 실행
```
...
| 종료 코드(Exit Code) | stdout | stderr | 툴 실행 | Claude가 보는 것 |
|-----------|--------|--------|----------------|-------------|
...
1. PreToolUse에서 Claude에게 메시지를 보낼 수 있는 **유일한 방법**
2. stderr 내용이 Claude에게 "자동으로 피드백"됨
3. Claude가 차단 메시지를 보고 다음 행동을 이해함
4. 툴 실행이 방지됨
