...
# This runs when Roo Codex finishes responding
...
cache_dir="$CODEX_PROJECT_DIR/.codex/tsc-cache/${session_id:-default}"
...
if [[ "$has_errors" == "true" ]]; then
    ...
    # Format message for Codex when using exit code 2
    if [[ $total_errors -ge 5 ]]; then
        ...
        # Exit with status 2 to send feedback to Codex
        exit 2
    else
        echo "" >&2
        ...
        # Exit with status 2 to send feedback to Codex for any errors
        exit 2
    fi
else
