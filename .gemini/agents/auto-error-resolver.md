...
1. 에러 체크 훅이 남긴 **에러 정보**를 확인합니다:
   - 에러 캐시 위치: `$GEMINI_PROJECT_DIR/.gemini/tsc-cache/[session_id]/last-errors.txt`
   - 영향받은 저장소 목록: `$GEMINI_PROJECT_DIR/.gemini/tsc-cache/[session_id]/affected-repos.txt`
   - TSC 명령 목록: `$GEMINI_PROJECT_DIR/.gemini/tsc-cache/[session_id]/tsc-commands.txt`
...
```bash
# 1. 에러 정보 읽기
cat "$GEMINI_PROJECT_DIR"/.gemini/tsc-cache/*/last-errors.txt

# 2. 어떤 TSC 명령을 써야 하는지 확인
cat "$GEMINI_PROJECT_DIR"/.gemini/tsc-cache/*/tsc-commands.txt
...
훅이 각 저장소에 대한 올바른 TSC 명령을 자동으로 감지해 저장합니다. 검증에 사용할 명령은 항상 `$GEMINI_PROJECT_DIR/.gemini/tsc-cache/*/tsc-commands.txt`를 확인하세요.
