...
- 훅에서 종료 코드 2를 반환, stderr → Claude
- Claude가 차단 메시지를 보고, 진행하려면 스킬을 사용해야 함
...
- Claude가 프롬프트를 보기 전에 리마인더를 주입
- Claude가 관련 스킬을 인지하도록 함
- 강제는 아니고 안내만 제공
...
- [ ] `.claude/skills/{name}/SKILL.md`에 스킬 파일 생성
...
1. 프론트매터를 포함해 `.claude/skills/{name}/SKILL.md` 생성
2. `.claude/skills/skill-rules.json`에 엔트리 추가
3. `npx tsx` 명령으로 테스트
