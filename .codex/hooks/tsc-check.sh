...
# Uses stderr for visibility in Roo Codex main interface

CODEX_PROJECT_DIR="${CODEX_PROJECT_DIR:-$HOME/project}"
HOOK_INPUT=$(cat)
SESSION_ID="${session_id:-default}"
CACHE_DIR="$HOME/.codex/tsc-cache/$SESSION_ID"
...
get_repo_for_file() {
    local file_path="$1"
    local relative_path="${file_path#$CODEX_PROJECT_DIR/}"
...
run_tsc_check() {
    local repo="$1"
    local repo_path="$CODEX_PROJECT_DIR/$repo"
...
find "$HOME/.codex/tsc-cache" -maxdepth 1 -type d -mtime +7 -exec rm -rf {} \; 2>/dev/null || true
