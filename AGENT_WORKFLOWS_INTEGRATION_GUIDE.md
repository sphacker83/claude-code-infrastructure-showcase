...
이 저장소는 Agent Workflows 인프라 구성요소를 모아둔 **레퍼런스 라이브러리**입니다. 사용자는 특정 조각을 프로젝트에 적용하려고 합니다. 당신의 역할은 다음과 같습니다.
...
```bash
cp -r /path/to/showcase/.agents/workflows/skills/[skill-name] \
      $GEMINI_PROJECT_DIR/.agents/workflows/skills/
```
...
**파일 존재 여부 확인:**
```bash
ls $GEMINI_PROJECT_DIR/.agents/workflows/skills/skill-rules.json
```
...
```bash
# 스킬 복사 확인
ls -la $GEMINI_PROJECT_DIR/.agents/workflows/skills/[skill-name]

# skill-rules.json 문법 확인
cat $GEMINI_PROJECT_DIR/.agents/workflows/skills/skill-rules.json | jq .
```
...
1. 시작점으로 스킬 복사
```bash
cp -r showcase/.agents/workflows/skills/frontend-dev-guidelines \
      $GEMINI_PROJECT_DIR/.agents/workflows/skills/vue-dev-guidelines
```
...
```bash
# 두 파일 복사
cp showcase/.agents/workflows/hooks/skill-activation-prompt.sh \
   $GEMINI_PROJECT_DIR/.agents/workflows/hooks/
cp showcase/.agents/workflows/hooks/skill-activation-prompt.ts \
   $GEMINI_PROJECT_DIR/.agents/workflows/hooks/

# 실행 권한 부여
chmod +x $GEMINI_PROJECT_DIR/.agents/workflows/hooks/skill-activation-prompt.sh

# 필요 시 의존성 설치
if [ -f "showcase/.agents/workflows/hooks/package.json" ]; then
  cp showcase/.agents/workflows/hooks/package.json \
     $GEMINI_PROJECT_DIR/.agents/workflows/hooks/
  cd $GEMINI_PROJECT_DIR/.agents/workflows/hooks && npm install
fi
```
...
```bash
# 파일 복사
cp showcase/.agents/workflows/hooks/post-tool-use-tracker.sh \
   $GEMINI_PROJECT_DIR/.agents/workflows/hooks/

# 실행 권한 부여
chmod +x $GEMINI_PROJECT_DIR/.agents/workflows/hooks/post-tool-use-tracker.sh
```
...
```bash
./.agents/workflows/hooks/tsc-check.sh
```
...
cp showcase/.agents/workflows/agents/[agent-name].md \
   $GEMINI_PROJECT_DIR/.agents/workflows/agents/
```
...
필요 시 치환 예시:
```bash
sed -i 's|~/git/old-project/|.|g' $GEMINI_PROJECT_DIR/.agents/workflows/agents/[agent].md
sed -i 's|/root/git/.*PROJECT.*DIR|$GEMINI_PROJECT_DIR|g' \
    $GEMINI_PROJECT_DIR/.agents/workflows/agents/[agent].md
```
...
```bash
cp showcase/.agents/workflows/commands/[command].toml \
   $GEMINI_PROJECT_DIR/.agents/workflows/commands/
```
...
```bash
# 1. 훅 실행 권한
ls -la $GEMINI_PROJECT_DIR/.agents/workflows/hooks/*.sh
# 기대값: -rwxr-xr-x
...
ls $GEMINI_PROJECT_DIR/.agents/workflows/hooks/node_modules/
# package.json이 있다면 패키지 목록이 보여야 함

# 4. settings.json 유효성
cat $GEMINI_PROJECT_DIR/.agents/workflows/settings.json | jq .
# 오류 없이 파싱되어야 함
```
...
현재 .agents/workflows/settings.json이 이미 있나요, 아니면 새로 만들까요?
```
...
별도 설정 없이 바로 사용 가능하므로 .agents/workflows/agents/로 복사하겠습니다.
```
