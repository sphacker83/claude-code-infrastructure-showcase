...
프로젝트 루트에 있는 `.claude/settings.json`을 생성하거나 업데이트하세요:
...
`.claude/hooks/error-handling-reminder.ts`에서 파일 카테고리 감지를 구성하세요:
...
3. **경로 확인:** `$CLAUDE_PROJECT_DIR`가 올바르게 설정되어 있는지 확인
4. **TypeScript 확인:** `cd .claude/hooks && npx tsc`를 실행해 에러가 있는지 확인
